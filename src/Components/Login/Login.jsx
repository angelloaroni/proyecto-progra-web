...
const Login = ({ onLogin, onGoRegister }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    setLoading(true);
    const resultado = await onLogin(codigo, password, rol);
    setLoading(false);
    if (!resultado.success) {
      setError(resultado.message || "Código o contraseña incorrectos, o cuenta inactiva.");
    }
  };
    ...
    <section className="login-section">
      <div className="box login-card">
        ...
        <p className="login-footer-text">
          ¿No tienes cuenta?{" "}
          <a onClick={onGoRegister} role="button" tabIndex={0}>Regístrate aquí</a>
        </p>
      </div>
    </section>
  );
